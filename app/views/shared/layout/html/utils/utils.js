...
function attachImages(occurrenceResults) {
    occurrenceResults.forEach(function(e) {
        let mediaLength = _.get(e, 'media.length', 0);
        e._images = [];
...
let precalculatedKeys = ['taxonKey', 'datasetKey', 'country', 'publishingCountry', 'publishingOrg'];
function isSimpleQuery(f) {
    ...
    for (let i = 0; i < precalculatedKeys.length; i++) {
        let type = precalculatedKeys[i];
