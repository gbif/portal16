'use strict';
let angular = require('angular');

angular
    .module('portal')
    .directive('count', function($http, $filter, $q) {
        return {
            restrict: 'A',
            link: function(scope, element, attrs) {
                let url = attrs.count;
                element.html('<span class="loading"></span>');

                let countPromise = $http.get(url, {
                    params: {
                        limit: 0,
                    },
                });

                let promise = (typeof attrs.subtract === 'undefined') ? countPromise : $q.all([countPromise, $http.get(attrs.subtract, {
                    params: {
                        limit: 0,
                    },
                })]).then(function(response) {
                    response[0].data.count = (response[0].data.count - response[1].data.count);
                    return response[0];
                });

                promise.then(function(response) {
                    let number = $filter('localNumber')(response.data.count, gb.locale);
                    element.html(number);
                    element.addClass('loaded');
                }).catch(function() {
                    // swallow errors//TODO how to handle this in a gerneal way. in some cases, the count is essential
                });
            },
        };
    });
