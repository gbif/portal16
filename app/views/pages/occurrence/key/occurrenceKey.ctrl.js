...
function occurrenceKeyCtrl(leafletData, env, moment, $http, hotkeys) {
    ...
    vm.updateMarkerMessage = function() {
        ...
        let message, elevation = '', coordinateUncertainty = '';
        ...
        if (vm.markerMessage.elevation) {
            let e = vm.markerMessage.elevation.elevation + 'm';
            let eAccuracy = vm.markerMessage.elevation.elevationAccuracy;
            let source = vm.markerMessage.elevation.source || '';
    ...
    function setMap(data) {
        ...
        if (data.footprintWKT && hasValidOrNoSRS(data)) {
            try {
               let geojsonGeometry = parseStringToWKTs(data.footprintWKT);
