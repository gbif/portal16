...
function becomePublisherCtrl($timeout, $q, $http, constantKeys, suggestEndpoints, Publisher, DirectoryParticipants, Node, NodeCountry) {
    ...
    vm.getPublisherSuggestions = function(val) {
        ...
        let publisherSuggestions = $http.get(suggestEndpoints.publisher, {
            params: {
                q: val,
                limit: 10,
            },
    ...
    vm.getCountrySuggestions = function() {
        let suggestions = $http.get('/api/country/suggest.json');
    ...
    function getContact(publisher) {
        ...
        let p = _.filter(publisher.contacts, function(e) {
    ...
    function setPinOnMap(evt) {
        let latLong = ol.proj.transform(evt.coordinate, 'EPSG:3857', 'EPSG:4326');
        $timeout(function() {
    ...
    vm.createOrganization = function() {
        let body = getBody();
        let creation = $http.post('/api/eoi/create', body);
