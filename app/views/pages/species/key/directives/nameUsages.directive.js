...
angular.module('portal').directive('nameUsages', nameUsagesDirective);
...
function nameUsagesDirective() {
    ...
    function nameUsagesCtrl(
        ...
        $scope.create = function(element) {
            vm.chartElement = element[0].querySelector(
                '.nameUsageChartContainer'
            );
                    ...
                    OccurrenceSearch.query({
                        ...
                        angular.forEach(vm.synonyms.results, function(s) {
                            var found = _.find(usages, function(u) {
                                return (
                                    u.name === s.key.toString() &&
                                    vm.species.rank === s.rank
                                );
                            });
                            if (found) {
                        ...
                        var totalMinusAccepted = _.reduce(
                            vm.synonymNameUsages,
                            function(sum, n) {
                        ...
                        vm.categories.push({
                            name: vm.species.scientificName,
                            key: vm.species.key
                        });

                        for (var i = 0; i < vm.synonymNameUsages.length; i++) {
                            vm.data.push(
                                parseInt(vm.synonymNameUsages[i].count)
                            );
                            vm.categories.push({
                                name: vm.synonymNameUsages[i].scientificName,
                                key: vm.synonymNameUsages[i].name
                            });
                        }
                        $translate('metrics.occurrences').then(function(res) {
                            vm.seriesName = res;
                            Highcharts.chart(
                                asPieChart({
                                    series: [{data: vm.data}],
                ...
                series: [
                    {
                        ...
                        point: {
                            events: {
                                click: function() {
                                    $state.go('occurrenceSearchTable', {
                                        taxon_key: this.options.key
                                    });
                                }
                            }
                        }
