...
function dataValidatorKeyCtrl($http, $stateParams, $state, $timeout, DwcExtension, Remarks, $location, $sessionStorage, validatorFeedbackService, env) {
    ...
    let sortIssues = function(issues) {
    ...
    function handleValidationResult(responseData) {
        ...
        angular.forEach(data.results, function(resourceResult) {
                let vmResourceResult = _.omit(resourceResult, 'issues');
...
return _.indexOf(vm.evaluationCategory, value.issueCategory);});
                ...
                if (resourceResult.termsFrequency) {
                    for (let i=0; i < resourceResult.termsFrequency.length; i++) {
                        let key = Object.keys(resourceResult.termsFrequency[i])[0];
                ...
                let issueBlock, issueSample;

               let unknownTermIssueSamples = _.map(_.filter(resourceResult.issues, function(i) {
                return i.issue === 'UNKNOWN_TERM'
               }), function(s) {
 return s.relatedData;});
            let duplicatedTermIssuesSamples = _.map(_.filter(resourceResult.issues, function(i) {
