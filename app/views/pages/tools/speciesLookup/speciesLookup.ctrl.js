...
function speciesLookupCtrl($http, $scope, hotkeys, SpeciesMatch, Species, speciesConstants) {
    ...
    vm.setItem = function(item, selected) {
      var fields = ['scientificName', 'key', 'matchType', 'status', 'rank',  'kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species', 'accepted', 'acceptedKey'];
    ...
    vm.countSpecies = function() {
      var keysOnly = vm.species.filter(function(e){return !!e.key}).map(function(e){
        return e.key;
      });
      var namesOnly = vm.species.filter(function(e){return !!e.key}).map(function(e){
        return e.scientificName;
      });
      $http.post('/api/tools/species-count/count', {taxonKeys: keysOnly, names: namesOnly, countryCode: 'DK'}, {}).then(function(response){
