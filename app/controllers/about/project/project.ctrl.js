...
router.get('/project/:requestedPath(*)', function(req, res, next) {
    ...
    request(cmsApi.urlLookup.url + requestedPath, function(e, r, b) {
        ...
        if (r.statusCode !== 200) {
            ...
            if (requestedPath == b.data[0].targetUrl) {
                var proseUrl = cmsApi.project.url + b.data[0].id;
                request(proseUrl, function(error, response, body) {
                    body = JSON.parse(body);
                    var proseContent = {
                        data: body.data[0],
                        images: body.data[0].image,
                        self: body.self,
                        _meta: {
                    ...
                    if (error) {
                        ...
                        try {
                            if (jsonOutput == true) {
                                ...
                                res.render('pages/about/project/project.nunjucks', proseContent);
                            }
